...
export class Component {
      ...
      enter(node, parent, prop, index) {
        ...
        if (names.length > 0) {
          names.forEach((name) => {
            ...
            if (
              declaration &&
              /** @type {import('estree').VariableDeclaration} */ (declaration)
                .kind === "const" &&
              !deep
            ) {
      ...
      enter(node, parent) {
        ...
        if (
          ...
          const names = extract_names(
            /** @type {import('estree').Node} */ (assignee)
          );
  ...
  warn_on_undefined_store_value_references(node, parent, prop, scope) {
    if (
      ...
      this.warn(
        /** @type {any} */ (node),
        compiler_warnings.non_top_level_reactive_declaration
      );
    }
    if (
      is_reference(
      ...
      if (name[0] === "$") {
        ...
        if (
          ...
          if (
            ...
            return this.error(
              /** @type {any} */ (node),
              compiler_errors.contextual_store
            );
          }
        }
      }
                    ...
                    case "ObjectPattern": {
                      ...
                      const handle_prop = (prop) => {
                        if (prop.type === "RestElement") {
                          ...
                          rename_identifiers(
                            /** @type {import('estree').Pattern} */ (prop.value)
                          );
                        }
                      };
  ...
  extract_reactive_declarations() {
    ...
    this.ast.instance.content.body.forEach((node) => {
      ...
      if (node.type === "LabeledStatement" && node.label.name === "$") {
        ...
        const { expression } =
          /** @type {import('estree').ExpressionStatement} */ (node.body);
        const declaration =
          expression &&
          /** @type {import('estree').AssignmentExpression} */ (expression)
            .left;
                  ...
                  if (
                    ...
                    (prop.key.name === "type" &&
                      [
                        ...
                        "Array",
                        "Object",
                      ].indexOf(/** @type {string} */ (prop.value.value)) ===
                        -1) ||
