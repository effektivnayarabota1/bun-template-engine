import { is_html, is_svg, is_void } from "shared";
    ...
    case "tokenlist": // if list of tokens, split each
      return (
        ...
        value
          .split(regex_any_repeated_whitespaces)
          .every(
            (token) => (schema.values || []).indexOf(token.toLowerCase()) > -1
          )
      );
    default:
      return false;
...
export default class Element extends Node {
      ...
      if (this.name === "textarea") {
        if (info.children.length > 0) {
          ...
          if (value_attribute) {
            component.error(
              value_attribute,
              compiler_errors.textarea_duplicate_value
            );
            return;
          }
  ...
  validate() {
    if (
      ...
      this.component.warn(
        this,
        compiler_warnings.component_name_lowercase(this.name)
      );
    }
    ...
    this.attributes.forEach((attribute) => {
      ...
      if (regex_illegal_attribute_character.test(name)) {
        return component.error(
          attribute,
          compiler_errors.illegal_attribute(name)
        );
      }
      if (name === "slot") {
        if (!attribute.is_static) {
          return component.error(
            attribute,
            compiler_errors.invalid_slot_attribute
          );
        }
        if (component.slot_outlets.has(name)) {
          return component.error(
            attribute,
            compiler_errors.duplicate_slot_attribute(name)
          );
          // this code was unreachable. Still needed?
          // component.slot_outlets.add(name);
        }
        if (
          !(parent.type === "SlotTemplate" || within_custom_element(parent))
        ) {
          return component.error(
            attribute,
            compiler_errors.invalid_slotted_content
          );
        }
      }
      // Warnings
  ...
  validate_attributes_a11y() {
    ...
    attributes.forEach((attribute) => {
      ...
      if (name.startsWith("aria-")) {
        if (invisible_elements.has(this.name)) {
          ...
          component.warn(
            attribute,
            compiler_warnings.a11y_aria_attributes(this.name)
          );
        }
        ...
        if (!aria_attribute_set.has(type)) {
          ...
          component.warn(
            attribute,
            compiler_warnings.a11y_unknown_aria_attribute(type, match)
          );
        }
        ...
        if (
          ...
          component.warn(
            attribute,
            compiler_warnings.a11y_aria_activedescendant_has_tabindex
          );
        }
      }
      // aria-role
      if (name === "role") {
        if (invisible_elements.has(this.name)) {
          ...
          component.warn(
            attribute,
            compiler_warnings.a11y_misplaced_role(this.name)
          );
        }
          ...
          value.split(regex_any_repeated_whitespaces).forEach(
            /** @param {import('aria-query').ARIARoleDefinitionKey} current_role */ (
              ...
              if (current_role && is_abstract_role(current_role)) {
                component.warn(
                  attribute,
                  compiler_warnings.a11y_no_abstract_role(current_role)
                );
              ...
              if (!is_parent_section_or_article) {
                const has_nested_redundant_role =
                  current_role ===
                  a11y_nested_implicit_semantics.get(this.name);
              ...
              if (
                ...
                if (has_interactive_handlers) {
                  component.warn(
                    this,
                    compiler_warnings.a11y_interactive_supports_focus(
                      current_role
                    )
                  );
                }
    ...
    if (
      ...
      if (
        tab_index &&
        (!tab_index.is_static || Number(tab_index.get_static_value()) >= 0)
      ) {
    ...
    if (typeof role_value === "string" && roles.has(role_value)) {
      ...
      const invalid_aria_props = new Set(
        aria.keys().filter((attribute) => !(attribute in props))
      );
      ...
      attributes
        ...
        .forEach((prop) => {
          if (
            invalid_aria_props.has(
              /** @type {import('aria-query').ARIAProperty} */ (prop.name)
            )
          ) {
            component.warn(
              prop,
    ...
    if (
      ...
      if (has_interactive_handlers) {
        component.warn(
          this,
          compiler_warnings.a11y_no_noninteractive_element_interactions(
            this.name
          )
        );
      }
    }
    const has_dynamic_role =
      attribute_map.get("role") && !attribute_map.get("role").is_static;
  ...
  validate_special_cases() {
    ...
    if (this.name === "a") {
      ...
      if (
        ...
        const href_static_value = href_attribute.get_static_value()
          ...
          : null;
        if (
          href_static_value === null ||
          href_static_value.match(/^(https?:)?\/\//i)
        ) {
      ...
      if (href_attribute) {
        ...
        if (
          ...
          component.warn(
            href_attribute,
            compiler_warnings.a11y_invalid_attribute(
              href_attribute.name,
              href_value
            )
          );
        ...
        if (!id_attribute_valid && !name_attribute_valid) {
          component.warn(
            this,
            compiler_warnings.a11y_missing_attribute("a", "an", "href")
          );
        }
      }
    } else {
    ...
    if (this.name === "input") {
      ...
      if (type && autocomplete) {
        ...
        if (!is_valid_autocomplete(autocomplete_value)) {
          component.warn(
            autocomplete,
            compiler_warnings.a11y_autocomplete_valid(
              type_value,
              autocomplete_value
            )
          );
    ...
    if (this.name === "label") {
        ...
        if (
          children.some(
            (child) =>
              ...
              (a11y_labelable.has(child.name) || child.name === "slot")
          )
        ) {
          return true;
        }
      ...
      if (!attribute_map.has("for") && !has_input_child(this.children)) {
        component.warn(
          this,
          compiler_warnings.a11y_label_has_associated_control
        );
      }
    }
    if (this.name === "video") {
      ...
      const track = this.children.find(
        /** @param {Element} i */ (i) => i.name === "track"
      );
      if (track) {
    ...
    if (a11y_distracting_elements.has(this.name)) {
      ...
      component.warn(
        this,
        compiler_warnings.a11y_distracting_elements(this.name)
      );
    }
    ...
    if (this.name === "figure") {
      ...
      if (index !== -1 && index !== 0 && index !== children.length - 1) {
        component.warn(
          children[index],
          compiler_warnings.a11y_structure_first_or_last
        );
      }
    }
    if (handlers_map.has("mouseover") && !handlers_map.has("focus")) {
      component.warn(
        this,
        compiler_warnings.a11y_mouse_events_have_key_events(
          "mouseover",
          "focus"
        )
      );
    ...
    if (handlers_map.has("mouseout") && !handlers_map.has("blur")) {
      component.warn(
        this,
        compiler_warnings.a11y_mouse_events_have_key_events("mouseout", "blur")
      );
    }
  }
      ...
      if (name === "value") {
        ...
        if (this.name === "select") {
          ...
          if (attribute && !attribute.is_static) {
            return component.error(
              attribute,
              compiler_errors.dynamic_multiple_attribute
            );
          }
        } else {
        ...
        if (!contenteditable) {
          return component.error(
            binding,
            compiler_errors.missing_contenteditable_attribute
          );
        ...
        return component.error(
          binding,
          compiler_errors.invalid_binding(binding.name)
        );
      }
    });
  }
  validate_content() {
    ...
    if (this.children.length === 0) {
      this.component.warn(
        this,
        compiler_warnings.a11y_missing_content(this.name)
      );
    }
  }
  validate_event_handlers() {
    ...
    this.handlers.forEach((handler) => {
      if (
        ...
        return component.error(
          handler,
          compiler_errors.invalid_event_modifier_combination(
            "passive",
            "preventDefault"
          )
        );
      ...
      handler.modifiers.forEach((modifier) => {
        if (!valid_modifiers.has(modifier)) {
          return component.error(
            handler,
            compiler_errors.invalid_event_modifier(
              list(Array.from(valid_modifiers))
            )
          );
        }
        if (modifier === "passive") {
          if (passive_events.has(handler.name)) {
            ...
            component.warn(
              handler,
              compiler_warnings.redundant_event_modifier_passive
            );
          }
        }
        if (
    ...
    if (class_attribute && !class_attribute.is_true) {
      if (
        ...
        /** @type {import('./Text.js').default} */ (
          class_attribute.chunks[0]
        ).data += ` ${id}`;
      } else {
        /** @type {import('./shared/Node.js').default[]} */ (
          class_attribute.chunks
        ).push(
          new Text(
            this.component,
            this,
  ...
  get can_use_textcontent() {
    return (
      ...
      this.children.every(
        (node) => node.type === "Text" || node.type === "MustacheTag"
      )
    );
  }
  get can_optimise_to_html_string() {
    const can_use_textcontent = this.can_use_textcontent;
    const is_template_with_text_content =
      this.name === "template" && can_use_textcontent;
    return (
  ...
  get can_optimise_hydration() {
    ...
    const is_static_text_content =
      this.is_static_content &&
      this.children.every((node) => node.type === "Text");
    return (
      this.can_optimise_to_html_string &&
      (this.can_use_innerhtml || is_static_text_content)
    );
  }
  hash() {
...
function should_have_attribute(node, attributes, name = node.name) {
  ...
  node.component.warn(
    node,
    compiler_warnings.a11y_missing_attribute(name, article, sequence)
  );
}
