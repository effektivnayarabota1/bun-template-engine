import { is_void } from "shared";
import {
  get_attribute_expression,
  get_attribute_value,
  get_class_attribute_value,
} from "./shared/get_attribute_value.js";
import { boolean_attributes } from "shared";
import {
...
export default function (node, renderer, options) {
  ...
  const style_expression =
    style_expression_list.length > 0 && x`{ ${style_expression_list} }`;
  if (node.attributes.some((attr) => attr.is_spread)) {
    ...
    node.attributes.forEach((attribute) => {
      ...
      if (name === "value" && node.name.toLowerCase() === "textarea") {
        ...
        renderer.add_expression(
          x`@add_styles(@merge_ssr_styles(${get_attribute_value(
            attribute
          )}, ${style_expression}))`
        );
      } else if (
        ...
        renderer.add_expression(
          x`@add_attribute("${attr_name}", ${snippet}, ${
  ...
  node.bindings.forEach((binding) => {
    ...
    if (name === "group") {
      const value_attribute = node.attributes.find(
        ({ name }) => name === "value"
      );
      if (value_attribute) {
        ...
        const condition =
          type === "checkbox"
            ? x`~${bound}.indexOf(${value})`
            : x`${value} === ${bound}`;
        renderer.add_expression(
          x`${condition} ? @add_attribute("checked", true, 1) : ""`
        );
      }
      ...
      if (node.name === "textarea") {
        ...
        if (value_attribute) {
          ...
          if (
            first &&
            first.type === "Text" &&
            regex_starts_with_newline.test(first.data)
          ) {
            renderer.add_string("\n");
          }
        }
  ...
  function add_tag_name() {
    if (node.tag_expr.node.type === "Literal") {
      ...
      renderer.add_expression(
        /** @type {import('estree').Expression} */ (node.tag_expr.node)
      );
    }
  }
}
