...
export default function tag(parser) {
  ...
  if (meta_tags.has(name)) {
    ...
    if (is_closing_tag) {
      ...
      if (parser.stack.length > 1) {
        parser.error(
          parser_errors.invalid_element_placement(slug, name),
          start
        );
      }
      ...
      if (parent.type !== "Element") {
        const error =
          ...
          parser.last_auto_closed_tag.tag === name
            ? parser_errors.invalid_closing_tag_autoclosed(
                name,
  ...
  const is_top_level_script_or_style =
    specials.has(name) && parser.stack.length === 1;
  let attribute;
  while (
    (attribute = read_attribute(
      parser,
      unique_names,
      is_top_level_script_or_style
    ))
  ) {
  ...
  if (name === "svelte:component") {
    ...
    if (
      ...
      parser.error(
        parser_errors.invalid_component_definition,
        definition.start
      );
    }
...
function read_attribute(parser, unique_names, is_static) {
  ...
  if (!is_static && type) {
    ...
    if (directive_name === "") {
      parser.error(
        parser_errors.empty_directive_name(type),
        start + colon_index + 1
      );
    }
  ...
  while (parser.index < parser.template.length) {
    ...
    if (done()) {
      ...
      if (parser.match("#")) {
        ...
        parser.error(
          parser_errors.invalid_logic_block_placement(location, name),
          index
        );
        ...
        const name = parser.read_until(/[^a-z]/);
        parser.error(
          parser_errors.invalid_tag_placement(location, name),
          index
        );
      }
